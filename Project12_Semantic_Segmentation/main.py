...
def load_vgg(sess, vgg_path):
    ...
    vgg_keep_prob_tensor_name = 'keep_prob:0' #dropout probability
    ...
    return w1, keep, vgg_layer3, vgg_layer4, vgg_layer7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    #Decoder for FCN-8 from FCN for Semantic Segmentation Long et al.
    # TODO: Implement function
    ...
    #Upsampling layer 7
    output1 = tf.layers.conv2d_transpose(conv_1x1_layer7, num_classes, 4, 2, 
                                         padding="same", kernel_initializer = tf.truncated_normal_initializer(stddev=0.01), 
                                         kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3)) 
    ...
    #skip connection 4 and 7
    skip_47 = tf.add(conv_1x1_layer4,output1)
        
    #Upsampling combined 4 and 7
    output2 = tf.layers.conv2d_transpose(skip_47, num_classes, 4, 2, 
                                         padding="same", kernel_initializer = tf.truncated_normal_initializer(stddev=0.01), 
                                         kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3)) 
    ...
    #skip connection between pool 3 and combined upsampled 4 and 7
    skip_3_47 = tf.add(conv_1x1_layer3,output2)
    
    #Final Upsampling
    # Not sure how the kernel size is selected
    #Check with kernel size = 8
    output3 = tf.layers.conv2d_transpose(skip_3_47, num_classes, 16, 8, 
                                         padding="same", kernel_initializer = tf.truncated_normal_initializer(stddev=0.01), 
                                         kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))     
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # TODO: Implement function
    #Get logits. Columns represent class, row represents pixel value
    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    correct_label = tf.reshape(correct_label, (-1, num_classes)) #each row for a column is logical true or false
